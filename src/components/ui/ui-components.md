...
- 컴포넌트: `src/utils/language.ts` (유틸리티)
- 페이지: `src/app/lab/ui-check/font-test/page.tsx`

#### FontTest 구현 세부사항

- 기능: 언어별 폰트 시스템 테스트 (자동 언어 감지, RTL 지원)
- 구현 방식: Unicode 범위 기반 언어 감지 + 멀티폰트 시스템
- 활용 라이브러리: React, Tailwind

#### FontTest 주요 특징

- **자동 언어 감지**: Unicode 범위 분석으로 한국어/영어/아랍어/혼합 텍스트 구분
- **멀티폰트 시스템**: Pretendard(한국어), Inter(영어), Noto Sans Arabic(아랍어) 자동 적용
- **RTL 지원**: 아랍어 등 RTL 언어의 텍스트 방향 자동 조정
- **실시간 테스트**: 샘플 텍스트 및 커스텀 입력으로 즉시 확인 가능
