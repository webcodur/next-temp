...
// 서비스
import { getResidentHistory } from '@/services/residents';
...
interface ResidentInstanceHistory {
  id: number;
  residentId: number;
  instanceId: number;
  memo?: string | null;
...
export default function ResidentHistorySection({ resident }: ResidentHistorySectionProps) {
  ...
  // #region 테이블 컬럼 정의
  const columns: BaseTableColumn<ResidentInstanceHistory>[] = [
    {
      header: '상태',
      key: 'isActive',
      align: 'center',
  ...
  if (!loading && historyData.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center py-12 text-center">
        ...
        <Button onClick={() => router.push(`/parking/occupancy/resident`)}>
          목록으로 돌아가기
        </Button>
      </div>
    );
