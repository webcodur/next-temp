              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => onTogglePrimary(userId)}
                    icon={CarFront}
                ...
                <TooltipContent>
                  <p>{`차량 소유자 설정 ${carUser?.isPrimary ? '(활성)' : '(비활성)'}`}</p>
                </TooltipContent>
              </Tooltip>
              {/* 차량 소유자 활성화 표시 */}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => onToggleAlarm(userId)}
                    icon={BellRing}
                    className={`w-12 h-12 min-w-12 text-white rounded-full transition-all duration-200 hover:scale-110 shadow-lg hover:shadow-xl border-2 [&_svg]:transition-transform [&_svg]:duration-200 [&_svg]:group-hover:scale-110 ${carUser?.carAlarm ? '[&_svg]:animate-pulse' : ''} ${
                      carUser?.carAlarm 
                ...
                <TooltipContent>
                  <p>{`알람 설정 ${carUser?.carAlarm ? '(활성)' : '(비활성)'}`}</p>
                </TooltipContent>
              </Tooltip>
              {/* 알람 활성화 표시 */}
              <div className={`absolute -top-1 -right-1 w-5 h-5 border-2 border-white rounded-full transition-all duration-200 shadow-md pointer-events-none ${
