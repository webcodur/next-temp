...
import { UserDetail, UserInstanceWithInstance } from '@/types/user';
...
const UserForm: React.FC<UserFormProps> = ({
  ...
  // edit 모드 전용 필드
  const editFields: GridFormFieldSchema[] = mode === 'edit' && user ? [
    {
      id: 'userId',
      label: '사용자 ID',
      rules: '시스템 자동 생성',
  ...
  // #region 현재 거주지 정보
  const currentUsers = user?.userInstance?.filter(ri => ri.instance) || [];
  
  const createUserFields = (user: UserInstanceWithInstance): GridFormFieldSchema[] => user?.instance ? [
    {
      id: 'instanceId',
      label: '세대 ID',
      component: (
        <SimpleTextInput
          value={`#${user.instance.id}`}
          onChange={() => {}}
          disabled={true}
          validationRule={getValidationRule('readonly', false)}
        />
    ...
    },
    {
      ...
      component: (
        <SimpleTextInput
          value={user.instance.address1Depth}
          onChange={() => {}}
          disabled={true}
          validationRule={getValidationRule('readonly', false)}
        />
    ...
    },
    {
      ...
      component: (
        <SimpleTextInput
          value={user.instance.address2Depth}
          onChange={() => {}}
          disabled={true}
          validationRule={getValidationRule('readonly', false)}
        />
    ...
    },
    {
      ...
      component: (
        <SimpleTextInput
          value={user.instance.address3Depth || '-'}
          onChange={() => {}}
          disabled={true}
          validationRule={getValidationRule('readonly', false)}
        />
      ...
      component: (
        <SimpleTextInput
          value={(() => {
            ...
            return typeMap[user.instance.instanceType as keyof typeof typeMap] || user.instance.instanceType;
          })()}
          onChange={() => {}}
          disabled={true}
          validationRule={getValidationRule('readonly', false)}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 현재 거주지 정보 */}
      <SectionPanel 
        title={`현재 거주지 정보 (${currentUsers.length}개)`}
        subtitle="사용자가 현재 거주하는 세대 정보입니다."
        icon={<Building size={18} />}
      >
        {currentUsers.length > 0 ? (
          <div className="space-y-6">
            {currentUsers.map((userInstance, index) => (
              <div key={userInstance.id} className="p-4 rounded-lg border bg-card">
                <h4 className="mb-3 text-sm font-medium text-muted-foreground">
                  거주지 #{index + 1}
                </h4>
                <GridFormAuto 
                  fields={createUserFields(userInstance)}
                  gap="16px"
