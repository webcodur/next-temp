...
import { Settings, GripVertical, Sliders, Edit3, Check, ChevronDown } from 'lucide-react';
...
const VehicleTypeCard: React.FC<VehicleTypeCardProps> = ({
  ...
  // #region 상태
  const [policies, setPolicies] = useState<VehicleAccessPolicy>(createEmptyAccessPolicy);
  ...
  // #region 핸들러
  const handlePolicyUpdate = (newPolicies: VehicleAccessPolicy) => {
  ...
  const handleOpenVehicleSettings = () => {
  ...
  return (
    ...
    >
      ...
      >
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              <button
                onClick={handleOpenVehicleSettings}
                disabled={isDragOverlay}
      ...
      <Modal
        ...
        onClose={() => setShowVehicleSettingsModal(false)}
        title={`${barrier.name} - 출입 유형 차량 설정`}
        size="md"
      >
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
          ...
          <div className="flex gap-3 justify-end pt-4 border-t border-border">
            <button
              onClick={() => setShowVehicleSettingsModal(false)}
              className="px-4 py-2 text-sm rounded-md neu-raised bg-background text-foreground hover:neu-flat"
            >
              닫기
            </button>
