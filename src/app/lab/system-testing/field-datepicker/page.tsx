/*
  파일명: src/app/lab/system-testing/field-datepicker/page.tsx
  기능: FieldDatePicker 컴포넌트의 다양한 옵션과 타입을 테스트하는 페이지
  책임: 단일, 범위, 날짜/시간, 월 선택 등 모든 종류의 DatePicker Field를 렌더링하고 상호작용을 확인한다.
*/
		...
		<div className="p-6 space-y-8 max-w-4xl mx-auto">
			...
			<div className="neu-flat p-6 rounded-lg space-y-4">
				...
				<div className="p-3 bg-muted/50 rounded text-sm space-y-1">
					<div>
						<strong>날짜+시간:</strong> {dateTimeValue?.toLocaleString() || '없음'}
					</div>
					<div>
						<strong>시간만:</strong> {timeValue?.toLocaleTimeString() || '없음'}
					</div>
				</div>
			</div>
			...
			<div className="neu-flat p-6 rounded-lg space-y-4">
				...
				<div className="p-3 bg-muted/50 rounded text-sm">
					<strong>선택된 월:</strong>{' '}
					{monthValue
						? `${monthValue.getFullYear()}년 ${monthValue.getMonth() + 1}월`
						: '없음'}
				</div>
			</div>
			...
			<div className="neu-flat p-6 rounded-lg space-y-4">
				...
				<div className="p-3 bg-muted/50 rounded text-sm space-y-1">
					<div>
						<strong>시작일:</strong> {rangeStartDate?.toLocaleDateString() || '없음'}
					</div>
					<div>
						<strong>종료일:</strong> {rangeEndDate?.toLocaleDateString() || '없음'}
					</div>
					<div>
						<strong>기간:</strong>{' '}
