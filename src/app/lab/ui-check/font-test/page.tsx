...
export default function FontTestPage() {
  ...
  const [selectedText, setSelectedText] = useState<'korean' | 'english' | 'arabic' | 'mixed'>('korean');
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="p-6 rounded-lg neu-flat">
        <h3 className="mb-4 text-lg font-semibold">{t('폰트테스트_샘플텍스트선택')}</h3>
      ...
      {/* #region 커스텀 텍스트 입력 */}
      <div className="p-6 rounded-lg neu-flat">
        <h3 className="mb-4 text-lg font-semibold">{t('폰트테스트_커스텀텍스트입력')}</h3>
        <textarea
          value={customText}
          onChange={(e) => setCustomText(e.target.value)}
          placeholder={t('폰트테스트_플레이스홀더')}
          className="p-4 w-full rounded-lg border border-gray-300 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
          rows={4}
        />
      </div>
      {/* #endregion */}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="p-6 rounded-lg neu-flat">
          <h3 className="mb-4 text-lg font-semibold">{t('폰트테스트_감지결과')}</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="font-medium">{t('폰트테스트_감지된언어')}:</span>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                detectedLang === 'ko' ? 'bg-blue-100 text-blue-800' :
                detectedLang === 'ar' ? 'bg-green-100 text-green-800' :
                detectedLang === 'en' ? 'bg-purple-100 text-purple-800' :
            ...
            <div className="flex justify-between">
              ...
              <span className="text-gray-600">{isRtl ? t('폰트테스트_방향_우좌') : t('폰트테스트_방향_좌우')}</span>
            </div>
          </div>
        </div>

        <div className="p-6 rounded-lg neu-flat">
          <h3 className="mb-4 text-lg font-semibold">{t('폰트테스트_폰트미리보기')}</h3>
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-600">MultiLang ({t('공통_자동선택')})</label>
              <p className={`font-multilang text-lg p-3 border rounded ${isRtl ? 'text-right' : 'text-left'}`}>
                {currentText}
              </p>
            </div>
            <div>
              <label className="text-sm font-medium text-gray-600">Pretendard ({t('공통_한국어전용')})</label>
              <p className="p-3 text-lg rounded border font-pretendard">
                {currentText}
              </p>
            </div>
            <div>
               <label className="text-sm font-medium text-gray-600">Inter ({t('공통_영어전용')})</label>
               <p className="p-3 text-lg rounded border font-inter">
                 {currentText}
               </p>
             </div>
             <div>
               <label className="text-sm font-medium text-gray-600">Cairo ({t('공통_아랍어전용')})</label>
      ...
      <div className="p-8 rounded-lg neu-inset">
        <h3 className="mb-4 text-lg font-semibold">{t('폰트테스트_실제적용결과')}</h3>
        <div 
