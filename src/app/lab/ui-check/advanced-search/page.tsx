			...
			<div className="mx-auto max-w-6xl">
				<div className="mb-8">
					<h1 className="mb-4 text-3xl font-bold text-gray-900">
						AdvancedSearch + Field 컴포넌트
					</h1>
					<p className="text-gray-600">
						기본 Field 컴포넌트를 고급 검색 패널에서 테스트
					</p>
				</div>
				...
				<div className="bg-white rounded-lg shadow-sm">
					<AdvancedSearch
						...
						searchLabel="검색 실행"
						resetLabel="필터 초기화"
						statusText={`선택된 태그: ${selectedTag || '없음'} | 상태: ${filterStatus || '전체'}`}>
						{/* 첫 번째 줄: 텍스트 필드들 */}
						<FieldText
							label="키워드 검색"
				...
				<div className="p-6 mt-8 bg-white rounded-lg shadow-sm">
					...
					<div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2 lg:grid-cols-3">
						...
						<div>
							...
							{categoryOptions.find((opt) => opt.value === categoryType)?.label}
						</div>
						<div>
							<span className="font-medium">인증 사용자:</span>{' '}
							{isVerified ? '예' : '아니오'}
						</div>
						<div>
							<span className="font-medium">알림:</span>{' '}
							{enableNotifications ? '활성' : '비활성'}
						</div>
						<div>
