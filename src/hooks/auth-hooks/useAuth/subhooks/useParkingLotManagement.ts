...
export function useParkingLotManagement() {
	...
	const selectParkingLot = useCallback(
		(parkingLotId: number) => {
		...
		[tokenSelectedParkingLotId, setManualSelectedParkingLotId, parkingLots]
	);
	...
	const getSelectedParkingLot = useCallback(() => {
		...
		const selectedLot =
			parkingLots.find((lot) => lot.id === effectiveSelectedParkingLotId) ||
			null;
