...
export function useCarResidentManager() {
  ...
  const loadCarResidentsWithDetails = useCallback(async (carId: number, instanceId?: number): Promise<CarResidentWithDetails[]> => {
    console.log(`차량 ID ${carId}에 연결된 주민 목록 조회 시작${instanceId ? `, 세대 ID ${instanceId}로 필터링` : ''}`);
  ...
  const disconnectResident = useCallback(async (residentId: number) => {
    ...
    if (!carResident && selectedCarInstanceId) {
      console.warn('로컬 상태에서 주민 연결 정보를 찾을 수 없어 재시도 필요...');
      return { success: false, errorMsg: '주민 연결 정보를 찾을 수 없습니다. 페이지를 새로고침 후 다시 시도해주세요.' };
    }
  ...
  const performPrimaryCarToggle = useCallback(async (
    ...
    if (newPrimaryState) {
      const currentPrimaryResidents = carResidents.filter(cr => 
        cr.id !== residentId && Boolean(cr.isPrimary)
      );
      
      // 기존 소유자들을 순차적으로 비활성화
      for (const primaryResident of currentPrimaryResidents) {
        await updateCarInstanceResident(primaryResident.carInstanceResidentId, {
    ...
    if (updateResult.success && updateResult.data) {
      ...
      const updatedCarResidents = carResidents.map(cr => ({
        ...
        carAlarm: cr.id === residentId ? updateResult.data.carAlarm : cr.carAlarm
      }));
  ...
  const confirmPrimaryCarTransfer = useCallback(async (
    ...
    const otherPrimaryResidents = carResidents.filter(cr => 
    ...
    try {
      ...
      const allPrimaryResidents = carResidents.filter(cr => Boolean(cr.isPrimary));
      ...
      for (const primaryResident of allPrimaryResidents) {
        ...
        if (!deactivateResult.success) {
          throw new Error(`기존 차량 소유자(${primaryResident.name}) 해제에 실패했습니다: ${deactivateResult.errorMsg}`);
        }
      }
  ...
  const togglePrimary = useCallback(async (residentId: number) => {
    ...
    if (newPrimaryState) {
      ...
      if (existingPrimaryResident) {
        ...
        try {
          const transferResult = await confirmPrimaryCarTransfer(existingPrimaryResident, residentId);
          return {
            success: transferResult.success,
            needsTransfer: false,
            errorMsg: transferResult.success ? undefined : '차량 소유자 전환에 실패했습니다.'
          };
        } catch (error) {
