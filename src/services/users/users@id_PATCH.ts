...
import { UpdateUserRequest, UserDetail } from '@/types/user';
import { getApiErrorMessage } from '@/utils/apiErrorMessages';

// #region 서버 타입 정의 (내부 사용)
interface UpdateUserServerRequest {
	name?: string;
	phone?: string;
	email?: string;
	birth_date?: string;
...
export async function updateUser(id: number, data: UpdateUserRequest) {
	...
	const response = await fetchDefault(`/users/${id}`, {
		method: 'PATCH',
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'updateUser'),
		};
	}
