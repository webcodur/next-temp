...
import {
	CreateUserInstanceRequest,
	UserDetail,
} from '@/types/user';
import { getApiErrorMessage } from '@/utils/apiErrorMessages';

// #region 서버 타입 정의 (내부 사용)
interface CreateUserInstanceServerRequest {
	user_id: number;
	instance_id: number;
	memo?: string;
}
...
interface UserDetailServerResponse {
	...
	created_at: string;
	updated_at: string;
	user_instance?: UserInstanceServerResponse[] | null;
}
// #endregion

// #region 변환 함수 (내부 사용)
function clientToServer(
	client: CreateUserInstanceRequest
): CreateUserInstanceServerRequest {
	return {
...
function serverToClient(server: UserDetailServerResponse): UserDetail {
	return {
		...
		userInstance:
			server.user_instance && Array.isArray(server.user_instance)
...
export async function createUserInstance(
	data: CreateUserInstanceRequest
) {
	const serverRequest = clientToServer(data);
	const response = await fetchDefault('/users/instances', {
		method: 'POST',
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'createUserInstance'),
		};
	}
