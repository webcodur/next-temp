...
import { SearchParkingDeviceHistoryRequest, ParkingDeviceHistoryListResponse, ParkingDeviceHistory } from '@/types/device';
...
// #region 변환 함수 (내부 사용)
function serverToClient(server: ParkingDeviceHistoryServerResponse): ParkingDeviceHistory {
  return {
    id: server.id,
...
function serverListToClient(serverResponse: ParkingDeviceHistoryListServerResponse): ParkingDeviceHistoryListResponse {
  return {
...
export async function searchParkingDeviceHistory(id: number, params?: SearchParkingDeviceHistoryRequest) {
  const searchParams = new URLSearchParams();
  
  if (params?.page) searchParams.append('page', params.page.toString());
  if (params?.limit) searchParams.append('limit', params.limit.toString());
  if (params?.actionType) searchParams.append('action_type', params.actionType);
  if (params?.changedFields) searchParams.append('changed_fields', params.changedFields);
  ...
  if (!response.ok) {
    const errorMsg = result.message || `차단기 변경 이력 조회 실패(코드): ${response.status}`;
