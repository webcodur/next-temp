...
function clientToServer(
	client: UpdateParkingDeviceRequest
): UpdateParkingDeviceServerRequest {
	return {
		name: client.name,
		ip: client.ip,
		port: client.port,
...
export async function updateParkingDevice(
	id: number,
	data: UpdateParkingDeviceRequest
) {
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: getApiErrorMessage('devices_update', result, response.status),
		};
	}
