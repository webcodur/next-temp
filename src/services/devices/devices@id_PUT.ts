...
import { UpdateParkingDeviceRequest, ParkingDevice } from '@/types/device';

// #region 서버 타입 정의 (내부 사용)
interface UpdateParkingDeviceServerRequest {
  name?: string;
  ip?: string;
  port?: string;
  server_port?: string;
...
// #region 변환 함수 (내부 사용)
function clientToServer(client: UpdateParkingDeviceRequest): UpdateParkingDeviceServerRequest {
  return {
    name: client.name,
    ip: client.ip,
    port: client.port,
...
export async function updateParkingDevice(id: number, data: UpdateParkingDeviceRequest) {
  ...
  if (!response.ok) {
    const errorMsg = result.message || `차단기 수정 실패(코드): ${response.status}`;
