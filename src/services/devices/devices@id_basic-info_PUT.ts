...
import {
	UpdateParkingDeviceBasicInfoRequest,
	ParkingDevice,
} from '@/types/device';
import { getApiErrorMessage } from '@/utils/apiErrorMessages';

// #region 서버 타입 정의 (내부 사용)
interface UpdateParkingDeviceBasicInfoServerRequest {
	name?: string;
	cctv_url?: string;
...
function clientToServer(
	client: UpdateParkingDeviceBasicInfoRequest
): UpdateParkingDeviceBasicInfoServerRequest {
	return {
		name: client.name,
...
export async function updateParkingDeviceBasicInfo(
	id: number,
	data: UpdateParkingDeviceBasicInfoRequest
) {
	const serverRequest = clientToServer(data);
	const response = await fetchDefault(`/devices/${id}/basic-info`, {
		method: 'PUT',
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'updateParkingDeviceBasicInfo'),
		};
	}
