...
import { UpdateParkingDeviceOperationRequest, ParkingDevice } from '@/types/device';

// #region 서버 타입 정의 (내부 사용)
interface UpdateParkingDeviceOperationServerRequest {
  device_type?: number;
  is_ticketing?: string;
...
// #region 변환 함수 (내부 사용)
function clientToServer(client: UpdateParkingDeviceOperationRequest): UpdateParkingDeviceOperationServerRequest {
  return {
...
export async function updateParkingDeviceOperation(id: number, data: UpdateParkingDeviceOperationRequest) {
  const serverRequest = clientToServer(data);
  const response = await fetchDefault(`/devices/${id}/operation`, {
    method: 'PUT',
  ...
  if (!response.ok) {
    const errorMsg = result.message || `차단기 운영 설정 수정 실패(코드): ${response.status}`;
