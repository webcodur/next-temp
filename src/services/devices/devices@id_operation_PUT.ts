...
import {
	UpdateParkingDeviceOperationRequest,
	ParkingDevice,
} from '@/types/device';
...
function clientToServer(
	client: UpdateParkingDeviceOperationRequest
): UpdateParkingDeviceOperationServerRequest {
	return {
...
export async function updateParkingDeviceOperation(
	id: number,
	data: UpdateParkingDeviceOperationRequest
) {
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: getApiErrorMessage('devices_operation_update', result, response.status),
		};
	}
