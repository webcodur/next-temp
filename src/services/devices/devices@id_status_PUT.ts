...
import { UpdateParkingDeviceStatusRequest, ParkingDevice } from '@/types/device';
...
// #region 변환 함수 (내부 사용)
function clientToServer(client: UpdateParkingDeviceStatusRequest): UpdateParkingDeviceStatusServerRequest {
  return {
...
export async function updateParkingDeviceStatus(id: number, data: UpdateParkingDeviceStatusRequest) {
  const serverRequest = clientToServer(data);
  const response = await fetchDefault(`/devices/${id}/status`, {
    method: 'PUT',
