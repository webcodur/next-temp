...
import { UpdateParkingDevicePermissionsRequest, ParkingDevice } from '@/types/device';
...
// #region 변환 함수 (내부 사용)
function clientToServer(client: UpdateParkingDevicePermissionsRequest): UpdateParkingDevicePermissionsServerRequest {
  return {
...
export async function updateParkingDevicePermissions(id: number, data: UpdateParkingDevicePermissionsRequest) {
  const serverRequest = clientToServer(data);
  const response = await fetchDefault(`/devices/${id}/permissions`, {
    method: 'PUT',
