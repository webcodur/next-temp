...
// #region 변환 함수 (내부 사용)
function serverToClient(server: VehicleAccessServerResponse): VehicleAccessResponse {
  return {
...
export async function checkVehiclePermission(id: number, vehicleType: VehicleType) {
  const response = await fetchDefault(`/devices/${id}/check-permission/${vehicleType}`, {
    method: 'GET',
  });
