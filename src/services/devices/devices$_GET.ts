...
function serverListToClient(
	serverResponse: ParkingDeviceListServerResponse
): ParkingDeviceListResponse {
	return {
...
export async function searchParkingDevices(
	params?: SearchParkingDeviceRequest
) {
	...
	if (params?.deviceType)
		searchParams.append('device_type', params.deviceType.toString());
	if (params?.status) searchParams.append('status', params.status.toString());
	if (params?.ip) searchParams.append('ip', params.ip);
	if (params?.taxiPermission !== undefined)
		searchParams.append('taxi_permission', params.taxiPermission.toString());
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: getApiErrorMessage('devices_search', result, response.status),
		};
	}
