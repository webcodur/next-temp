...
function clientToServer(
	client: CreateResidentInstanceRequest
): CreateResidentInstanceServerRequest {
	return {
...
function serverToClient(server: ResidentDetailServerResponse): ResidentDetail {
	return {
		...
		residentInstance:
			server.resident_instance && Array.isArray(server.resident_instance)
...
export async function createResidentInstance(
	data: CreateResidentInstanceRequest
) {
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'createResidentInstance'),
		};
	}
