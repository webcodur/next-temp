...
function clientToServer(
	client: UpdateResidentInstanceRequest
): UpdateResidentInstanceServerRequest {
	return {
		memo: client.memo,
	};
}
...
export async function updateResidentInstance(
	id: number,
	data: UpdateResidentInstanceRequest
) {
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: getApiErrorMessage('residents_instances_update', result, response.status),
		};
	}
