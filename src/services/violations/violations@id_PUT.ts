...
function clientToServer(
	client: UpdateCarViolationRequest
): UpdateCarViolationServerRequest {
	return {
...
export async function updateViolation(
	id: number,
	data: UpdateCarViolationRequest
) {
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'updateViolation'),
		};
	}
