...
export async function searchViolations(params?: SearchCarViolationRequest) {
	...
	if (params?.violationType)
		searchParams.append('violation_type', params.violationType);
	if (params?.status) searchParams.append('status', params.status);
	if (params?.reporterType)
		searchParams.append('reporter_type', params.reporterType);
	if (params?.isProcessed !== undefined)
		searchParams.append('is_processed', params.isProcessed.toString());
	if (params?.violationTimeFrom)
		searchParams.append('violation_time_from', params.violationTimeFrom);
	if (params?.violationTimeTo)
		searchParams.append('violation_time_to', params.violationTimeTo);
	...
	if (!response.ok) {
		const errorMsg =
			result.message ||
			`차량 위반 기록 목록 조회 실패(코드): ${response.status}`;
