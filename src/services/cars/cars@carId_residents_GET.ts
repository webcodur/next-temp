...
function serverToClient(
	server: CarResidentServerResponse
): CarResidentWithDetails {
	return {
		id: server.id, // 주민 ID
		name: server.name,
		phone: server.phone,
...
export async function getCarResidents(
	carId: number,
	instanceId: number,
	parkinglotId?: string
) {
	...
	const response = await fetchDefault(
		`/cars/${carId}/residents/${instanceId}`,
		{
			method: 'GET',
			headers,
		}
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'getCarResidents'),
		};
	}
