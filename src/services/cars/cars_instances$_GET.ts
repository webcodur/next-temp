...
import { SearchCarInstanceParams, CarInstanceListResponse, CarInstanceResidentDetail } from '@/types/car';
...
// #region 변환 함수 (내부 사용)
function serverToClient(server: CarInstanceResidentDetailServerResponse): CarInstanceResidentDetail {
  return {
    id: server.id,
...
export async function searchCarInstances(params?: SearchCarInstanceParams, parkinglotId?: string) {
  ...
  const url = `/cars/instances${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
  ...
  const clientResponse: CarInstanceListResponse = {
