...
export async function createCarInstanceResident(data: CreateCarInstanceResidentRequest, parkinglotId?: string) {
  ...
  if (!response.ok) {
    try {
      ...
      const errorMsg = await getApiErrorMessage(result, response.status, 'createCarInstanceResident');
      console.error('차량-주민 연결 생성 실패:', result);
      return { success: false, errorMsg };
    } catch (parseError) {
      // JSON 파싱 실패 시 기본 에러 메시지
      const errorMsg = await getApiErrorMessage({}, response.status, 'createCarInstanceResident');
