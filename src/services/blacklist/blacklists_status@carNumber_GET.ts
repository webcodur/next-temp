...
function serverToClient(
	server: BlacklistStatusServerResponse
): BlacklistStatusResponse {
	return {
		...
		blacklist: server.blacklist
			? {
					...
					registrationReason: server.blacklist
						.registration_reason as ENUM_BlacklistRegistrationReason,
...
export async function getBlacklistStatus(carNumber: string) {
	const response = await fetchDefault(
		`/blacklists/status/${encodeURIComponent(carNumber)}`,
		{
			method: 'GET',
		}
	);
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'getBlacklistStatus'),
		};
	}
