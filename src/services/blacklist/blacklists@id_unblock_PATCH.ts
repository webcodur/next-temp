...
function clientToServer(
	client: UnblockBlacklistRequest
): UnblockBlacklistServerRequest {
	return {
...
export async function unblockBlacklist(
	id: number,
	data: UnblockBlacklistRequest
) {
	...
	if (!response.ok) {
		return { 
			success: false, 
			errorMsg: await getApiErrorMessage(result, response.status, 'unblockBlacklist'),
		};
	}
