import { ApiProperty } from '@nestjs/swagger';
import {
  IsString,
  IsOptional,
  IsInt,
  IsUrl,
  Min,
  IsIn,
  IsNumber,
} from 'class-validator';

export class UpdateParkingDeviceStatusDto {
  @ApiProperty({
    description: '차단기 상태',
    example: 1,
    required: false,
  })
  @IsNumber()
  @IsOptional()
  status?: number;
}

export class UpdateParkingDeviceBasicInfoDto {
  @ApiProperty({
    description: '차단기 이름',
    example: '정문 입구 차단기',
    required: false,
  })
  @IsString()
  @IsOptional()
  name?: string;

  @ApiProperty({
    description: 'CCTV 주소',
    example: 'http://192.168.1.100:554/stream',
    required: false,
  })
  @IsUrl()
  @IsOptional()
  cctv_url?: string;

  @ApiProperty({
    description: '용지 부족 알림 받을 전화번호',
    example: '010-1234-5678',
    required: false,
  })
  @IsString()
  @IsOptional()
  representative_phone?: string;

  @ApiProperty({
    description: '순서 (KOP 메인 화면에서 보이는 순서)',
    example: 1,
    required: false,
  })
  @IsInt()
  @Min(0)
  @IsOptional()
  sequence?: number;
}

export class UpdateParkingDeviceNetworkDto {
  @ApiProperty({
    description: 'IP 정보',
    example: '192.168.1.100',
    required: false,
  })
  @IsString()
  @IsOptional()
  ip?: string;

  @ApiProperty({
    description: 'PORT 정보',
    example: '8000',
    required: false,
  })
  @IsString()
  @IsOptional()
  port?: string;

  @ApiProperty({
    description: '통합 보드 PORT',
    example: '8001',
    required: false,
  })
  @IsString()
  @IsOptional()
  server_port?: string;
}

export class UpdateParkingDevicePermissionsDto {
  @ApiProperty({
    description: '입주차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 1,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  resident_permission?: number;

  @ApiProperty({
    description: '정기차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 1,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  regular_permission?: number;

  @ApiProperty({
    description: '방문차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 1,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  visitor_permission?: number;

  @ApiProperty({
    description: '임시차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 1,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  temp_permission?: number;

  @ApiProperty({
    description: '업무차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 1,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  business_permission?: number;

  @ApiProperty({
    description: '상가차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 1,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  commercial_permission?: number;

  @ApiProperty({
    description: '택시 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 0,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  taxi_permission?: number;

  @ApiProperty({
    description: '발권차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 1,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  ticket_machine_permission?: number;

  @ApiProperty({
    description: '미등록차 출입 가능 여부 (0: 비허용, 1: 허용)',
    enum: [0, 1],
    example: 0,
    required: false,
  })
  @IsIn([0, 1])
  @IsOptional()
  unregistered_permission?: number;
}

export class UpdateParkingDeviceOperationDto {
  @ApiProperty({
    description: '차단기 타입 (1: 라즈베리파이, 2: 통합보드)',
    enum: [1, 2],
    example: 1,
    required: false,
  })
  @IsIn([1, 2])
  @IsOptional()
  device_type?: number;

  @ApiProperty({
    description: '발권기 사용 여부',
    enum: ['Y', 'N'],
    example: 'Y',
    required: false,
  })
  @IsIn(['Y', 'N'])
  @IsOptional()
  is_ticketing?: string;

  @ApiProperty({
    description: '정산기 사용 여부',
    enum: ['Y', 'N'],
    example: 'N',
    required: false,
  })
  @IsIn(['Y', 'N'])
  @IsOptional()
  is_receipting?: string;
}
