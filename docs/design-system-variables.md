...
| **주요 사용처** | • 토스트/알림 • 입력 검증 성공 메시지 • 그래프·차트의 긍정 데이터 • 배지/라벨(성공) • 프로그레스바·스텝 완료 표시 |
| **권장 조합** | `background: hsl(var(--success));` + `color: hsl(var(--success-foreground));` |
| **접근성** | WCAG 2.1 대비: 텍스트 대비 4.5:1 이상 확보. `--success` 배경 위 텍스트는 반드시 `--success-foreground` 사용. |
| **주의사항** | 1) 브랜드 컬러보다 우선순위 낮다. 2) 상태 표현 이외 용도로 남용 금지. |
